...
Below, I created a histogram of the chocolate bar rating scores.
I decided to use 25 bins, since this ensures that each bin represents
the frequency of only one numerical rating. Additionally, the bars in
the histogram are centered on the rating they represent, making it
easier to interpret.
```{r}
...
### 1.2. Count reviews by country of bean origin
...
same set of reviews, also calculate (1) the total number of reviews and 
(2) the standard deviation of the rating scores. Your answer should be a 
new data frame with these three summary statistics in three columns. Label 
the name of these columns mean, sd, and total.
```{r}
...
Which company manufacturer makes the best chocolate (or has the
highest ratings on average) with beans from Ecuador?
```{r}
chocolate.beans_from_equador %>%
...
Calculate the average rating across all country of origins for
...
Following up, remove any countries of bean origins that have less 
than 10 chocolate bar reviews. Now, which top 3 countries have 
the highest ratings on average?
```{r}
mean_rating_by_bean_origin %>%
  filter(n_reviews >= 10)
```
...
violin.ratings_by_continent <-
  ggplot(data = chocolate.with_cont.cobo_geq_10, aes(x = continent, y = rating)) +
...
Create a new set of columns titled beans, sugar, cocoa_butter,
...
Create a new set of columns titled char_cocoa, char_sweet,
char_nutty, char_creamy, char_roasty, char_earthy that contain a 1
or 0 representing whether or not that the most memorable
...
that review in the char_sweet column (Hint: check out str_detect()
...
For each year (i.e. review_date), calculate the mean value in each
new column you created across all reviews for that year. (Hint: If
all has gone well thus far, you should have a dataset with 16 rows
and 13 columns).
```{r}
...
This is the new-and-improved plot:
```{r fig.width = 10, fig.height = 7}
chocolate %>%
    ggplot(aes(
        x = as.factor(review_date),
  ...
  geom_jitter(size = 0.3, width = 0.075, alpha = 0.35) +
  labs(title = "Chocolate Ratings by Year of Review") +
  xlab("Year of Review") +
  ylab("Rating") +
